...
async function sendViaRelay( conn, bolt11 ) {
  ...
  return new Promise( ( resolve, reject ) => {
    ...
    let settled = false;
    let eventId = null; // set after event is finalized
    ...
    ws.addEventListener( 'open', async () => {
      ...
      if ( encryption === 'nip44' ) {
        encryptedContent = nip44.encrypt( payload, nip44.getConversationKey( clientSecretBytes, walletPubkey ) );
      } else {
        encryptedContent = await nip04.encrypt( conn.clientSecret, walletPubkey, payload );
      }
